    ...
    <v-form class="newadd-form" direction="horizontal">
      <v-form-item>
        <v-button
          ...
          html-type="button"
          @click="showModel"
          v-if="isAddSpecial===true"
        >添加礼品专场</v-button>
        <v-button type="primary" html-type="button" @click="showModel" v-else disabled>添加礼品专场</v-button>
      </v-form-item>
    </v-form>
    ...
    getData() {
      ...
      return _this.$http
        ...
        .then(res => {
          ...
          if (ary.length > 0) {
            _this.isAddSpecial = false;
          } else {
            _this.isAddSpecial = true;
          }
          _this.dataTotal = res.data.count;
...
<style src="./giftsale.less" lang="less"></style>

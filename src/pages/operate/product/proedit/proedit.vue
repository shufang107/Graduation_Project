      ...
      <v-spin :spinning="spinning" tip="加载中">
        <v-content>
          <v-form class="search-form" :model="specialSGForm" ref="specialrefSGForm">
            ...
            <!--未添加事件-->
            <!-- <v-form-item>
              <v-button type="primary" html-type="button">导出商品列表</v-button>
            </v-form-item>
            <v-form-item>
              <v-button type="primary" html-type="button">导入添加商品</v-button>
            </v-form-item> -->
            <v-form-item>
              ...
              <div class="data-gird" :style="`height:${gridHeight}`">
                <v-data-table :data="getData" :columns="faColumns" :currentData="faillist">
                  <template slot="td" slot-scope="props">
                    ...
                    <div v-else-if="props.column.field =='spgRestStock'">   <!--可售库存-支付的库存-->
                      <!-- {{props.item.spgSellStock - props.item.spgPayStock}} -->
                      {{props.item.spgSellStock}} {{props.item.spgPayStock}}
                    </div>
                    <span v-else v-html="props.content"></span>
...
export default {
  ...
  created() {
    ...
    (this.suColumns = [
      ...
      { title: "实际库存", field: "availableQuantity" }, //实际库存更改

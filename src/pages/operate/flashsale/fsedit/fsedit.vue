<template>
  <div class="operate-flashsale-fsedit-wrap">
    <v-modal
      class="modal-whole"
      :width="1200"
      title="编辑主题专场"
    ...
    >
      <v-spin :spinning="spinning" tip="加载中">
        <v-content>
          <v-form class="search-form" :model="specialFSForm" ref="specialrefFSForm">
            <v-form-item label="专场中文名">
              <v-input
                placeholder="请输入专场中文名"
                v-model="specialFSForm.spTitleCn"
                :value="specialFSForm.spTitleCn"
              ></v-input>
            </v-form-item>
            <v-form-item label="专场英文名">
              <v-input
                placeholder="请输入专场英文名"
                v-model="specialFSForm.spTitleEn"
                :value="specialFSForm.spTitleEn"
              ></v-input>
            </v-form-item>
            <v-form-item label="专场权重">
              <v-input
                placeholder="请输入专场权重值"
                v-model="specialFSForm.spWeight"
                :value="specialFSForm.spWeight"
              ></v-input>
            </v-form-item>
            <v-form-item label="开始时间">
              <v-date-picker
                clearable
                show-time
                v-model="specialFSForm.startTime"
                :value="specialFSForm.startTime"
              ></v-date-picker>
            </v-form-item>
            <v-form-item label="结束时间">
              <v-date-picker
                clearable
                show-time
                v-model="specialFSForm.endTime"
                :value="specialFSForm.endTime"
              ></v-date-picker>
            </v-form-item>
            <v-form-item label="副标题中文名">
              <v-input
                placeholder="请输入副标题中文名"
                v-model="specialFSForm.spSubTitleCn"
                :value="specialFSForm.spSubTitleCn"
              ></v-input>
            </v-form-item>
            <v-form-item label="副标题英文名">
              <v-input
                placeholder="请输入副标题英文名"
                v-model="specialFSForm.spSubTitleEn"
                :value="specialFSForm.spSubTitleEn"
              ></v-input>
            </v-form-item>
            <v-form-item label="主题背景颜色值">
              <v-input
                placeholder="请输入颜色值"
                v-model="specialFSForm.spColor"
                :value="specialFSForm.spColor"
              ></v-input>
            </v-form-item>
            <br>
            ...
            <v-form-item label="背景图URL(CN)(750px*N)">
              <v-input
                ...
                v-model="specialFSForm.spBgImgCn"
            ...
            <v-form-item label="背景图URL(EN)(750px*N)">
              <v-input
                ...
                v-model="specialFSForm.spBgImgEn"
        ...
        <div class="data-gird">
          <v-tabs class="fsedit-tab" active-tab-key="key1">
              ...
              <div class="data-gird" :style="`height:${gridHeight}`">
                <v-data-table :data="getData" :columns="suColumns" :currentData="successlist">
                  <template slot="td" slot-scope="props">
                    <div v-if="props.column.field=='availableQuantity'">
...
export default {
  ...
  methods: {
    ...
    downloadtemp() {
      console.log(setting.downLoadExcel + new Date().getTime());
    ...
    handleOK() {
      ...
      let params = {
        ...
        spBannerBgImgCn: this.specialFSForm.spBgImgCn,
        spBannerBgImgEn: this.specialFSForm.spBgImgEn,
...
<style src="./fsedit.less" lang="less"></style>
